...
type
  ...
  OggHeader = packed record
    {0-3}      ID:     array [1..4] of char;
    { Always "OggS" }
    {4}        StreamVersion: byte;
    { Stream structure version }
    {5}        TypeFlag: byte;
    { Header type flag }
    {6-13}     AbsolutePosition: int64;
    { Absolute granule position }
    {14-17}    Serial: integer;
    { Stream serial number }
    {18-21}    PageNumber: integer;
    { Page sequence number }
    {22-25}    Checksum: integer;
    { Page checksum }
    {26}       Segments: byte;
    { Number of page segments }
    {27, ...}  LacingValues: array [1..$FF] of byte;
    { Lacing values - segment sizes }
  end;
...
function GetOggInfo(const FileName: WideString; var Info: FileInfo;
  DecodeComments: boolean): boolean;
var
...
begin
  ...
  try
    ...
    if AnsiStrLComp(PChar(VORBIS_PARAMETERS_ID), Data,
    ...
    if AnsiStrLComp(PChar(SPEEX_PARAMETERS_ID), Data,
...
begin
  ...
  Value := Value + NewTagSize - (Info.TagEndPos - Info.SPagePos -
    Info.SPage.Segments - OggHeaderSize_27);
...
begin
  ...
  Destination.Seek(Info.SPagePos + Info.SPage.Segments + OggHeaderSize_27,
    soFromBeginning);
  for Index := 1 to Info.SPage.Segments do
...
begin
  ...
  try
    ...
    if Destination <> nil then
      Destination.Free;
    { Access error }
    if WideFileExists(BufferName) then
      WideDeleteFile(BufferName);
    end;
end;
